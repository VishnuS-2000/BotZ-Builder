...
export default function Home() {
  ...
  const createBot=async(e)=>{
    ...
    const status=await updateDoc(docRef,{bot:{name:botName,createdAt:serverTimestamp()}})
  ...
  const deleteBot=async(e)=>{
    const status=await updateDoc(docRef,{bot:{}})
...
const Modal=()=>{
  ...
  return <div className='flex justify-center items-center fixed bg-black/50 bg-blur-3xl w-screen h-screen top-0 bottom-0 left-0 right-0'>
      ...
      <div className='bg-gray-50 p-10 rounded-lg'>
          ...
          <TextField label="Bot Name" fullWidth value={botName} onChange={(e)=>{setBotName(e.target.value)}}/>
