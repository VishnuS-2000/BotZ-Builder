...
const IntentCard = ({tag, pattern, response,actions,id}: {tag: String, pattern: String, response: String}) => {
    return <div className="flex py-4 px-8 rounded-md my-8 w-full justify-around shadow-lg">
        <p>{tag}</p>
        <p>{pattern}</p>
        <p>{response}</p>
        <div className="flex items-center justify-center">
            <div className="rounded-full h-8 w-8 flex items-center justify-center cursor-pointer bg-white hover:bg-[#1e90ff33]" onClick={()=>{actions.edit}}>
